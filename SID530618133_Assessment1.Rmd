...
Clearly the Temperature was higher on average outside the Tropical Mid latitude than inside from 2014-2017. This is confusing though because you'd think it would be the other way round. I can use it as a template now though.

```{r}
tsa_model <- countryDataYears[, c("Reef.Name", "Latitude.Degrees", "Longitude.Degrees", "Temperature_Mean")]
tsa_model %>% distinct(Reef.Name, Latitude.Degrees, Longitude.Degrees, Temperature_Mean)
...
geom_point(data = tsa_model, alpha = 0.2, aes(y=Latitude.Degrees, x= Longitude.Degrees,
  size=Temperature_Mean>301, color = in_zone)) + scale_colour_viridis_c() + theme_minimal()
```
Checking Averages
